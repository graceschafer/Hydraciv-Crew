# Setting up email: instant job emails & weekly timesheets
...
2. **Monday timesheet email** (6am) — one email per crew member with their hours for the week that just finished, sent only to `grace@hydraciv.com.au` and `andrew@hydraciv.com.au`. The crew themselves are never emailed by this.

There are no reminder emails sent to crew — the only recipients of anything, ever, are the office addresses above.

Step 2 runs on a schedule with no browser open, so it lives in a Firebase Cloud Function (`functions/`) rather than in the app itself. That needs the Firebase project on the **Blaze (pay-as-you-go)** plan — see below; at this volume of email it should cost nothing or close to it, Cloud Scheduler's free tier covers the first 3 scheduled jobs and we use exactly 1.
...
3. **Create one template**: Email Templates → Create New Template. This one template is reused for both email types (instant job, timesheet) — the app sends fully-formed HTML, so the template itself just needs to pass it through:
   - **To email**: `{{to_email}}`
...
That creates one scheduled job (in Australia/Melbourne time):

| Function | Schedule | What it does |
|---|---|---|
| `mondayTimesheetEmail` | 6am, Monday | Sends Grace & Andrew each crew member's timesheet for the week that just finished — one email per person, none of it goes to the crew |

## 5. Test before trusting it

- In the Firebase Console → Functions, `mondayTimesheetEmail` has a matching entry in **Cloud Scheduler** — open it and hit **Force run** to trigger it immediately rather than waiting for Monday, then check your (and Andrew's) inbox and the function's logs.
