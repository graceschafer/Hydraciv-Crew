...
// Hydraciv Crew Portal — scheduled reminder & timesheet emails.
//
// Three scheduled jobs, all in Australia/Melbourne time:
//   1. dailyLogReminder     — 4pm Mon–Fri: nags anyone with no job-log dated today
//   2. sundayLogReminder    — 5pm Sunday:  polite blanket reminder to make sure the week is logged
//   3. mondayTimesheetEmail — 6am Monday:  sends each crew member last week's timesheet, cc'd to the office
//
...
async function getActiveCrewWithEmail() {
  const snap = await db.collection("crew").get();
  return snap.docs.map((d) => ({ id: d.id, ...d.data() })).filter((c) => c.active !== false && c.email);
}

// ---------- 1. Daily 4pm reminder (Mon–Fri): nudge anyone with no job-log dated today ----------
exports.dailyLogReminder = onSchedule(
  { schedule: "0 16 * * 1-5", timeZone: TIMEZONE, secrets: [EMAILJS_PRIVATE_KEY] },
  async () => {
    const today = dateToStr(nowInMelbourne());
    const crew = await getActiveCrewWithEmail();
    if (!crew.length) return;

    const logsSnap = await db.collection("job-logs").where("date", "==", today).get();
    const loggedToday = new Set();
    logsSnap.docs.forEach((d) => (d.data().employees || []).forEach((name) => loggedToday.add(name)));

    const toRemind = crew.filter((c) => !loggedToday.has(c.name));
    for (const c of toRemind) {
      const html = brandedEmailShell({
        header:
          '<h1 style="margin:0;font-size:22px;font-weight:800;color:#fff;">Log today’s job</h1>' +
          '<p style="margin:6px 0 0;font-size:13px;color:#cfe6f4;">' + esc(fmtDayLabel(today)) + "</p>",
        body:
          '<p style="font-size:14px;line-height:1.6;">Hi ' + esc(c.name) + ", looks like you haven’t logged a job sheet yet today. " +
          "If you’re still on the tools, no rush — but if you’re wrapping up, jump into the Crew Portal and log it before you knock off.</p>",
      });
      try {
        await sendViaEmailJS({ toEmail: c.email, subject: "Don’t forget to log today’s job", htmlBody: html });
      } catch (err) {
        logger.error("dailyLogReminder failed for " + c.name, err);
      }
    }
  }
);

// ---------- 2. Sunday reminder: polite blanket nudge to make sure the week's jobs are all in ----------
exports.sundayLogReminder = onSchedule(
  { schedule: "0 17 * * 0", timeZone: TIMEZONE, secrets: [EMAILJS_PRIVATE_KEY] },
  async () => {
    const crew = await getActiveCrewWithEmail();
    for (const c of crew) {
      const html = brandedEmailShell({
        header:
          '<h1 style="margin:0;font-size:22px;font-weight:800;color:#fff;">Week wrap-up</h1>' +
          '<p style="margin:6px 0 0;font-size:13px;color:#cfe6f4;">A quick Sunday check-in</p>',
        body:
          '<p style="font-size:14px;line-height:1.6;">Hi ' + esc(c.name) + ", just a friendly reminder to make sure every job you worked on this week has been logged in the Crew Portal. " +
          "Timesheets go out first thing Monday, so anything missing now will be missing from that.</p>",
      });
      try {
        await sendViaEmailJS({ toEmail: c.email, subject: "Quick check — got all your jobs logged this week?", htmlBody: html });
      } catch (err) {
        logger.error("sundayLogReminder failed for " + c.name, err);
      }
    }
  }
);

// ---------- 3. Monday morning timesheet email, per crew member, cc'd to Grace & Andrew ----------
    ...
    for (const c of crew) {
      ...
      try {
        await sendViaEmailJS({
          toEmail: c.email,
          ccEmail: OFFICE_EMAILS.join(","),
          subject: "Your weekly timesheet — " + fmtShortLabel(wk.startStr) + " to " + fmtShortLabel(wk.endStr),
          htmlBody: html,
        });
      } catch (err) {
